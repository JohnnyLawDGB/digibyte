...
`src/kernel/chainparams.cpp` populates `vOracleNodes` for each network. Each entry contains a compressed pubkey, an `endpoint` string, and an `is_active` flag. Mainnet and testnet25 declare 35 metadata slots, but only slots 0-16 are in `consensus.vOraclePublicKeys` and participate in the launch V1 quorum; slots 17-34 are inactive reserve metadata that require a future coordinated software update before they can sign consensus bundles. Regtest declares 7.
