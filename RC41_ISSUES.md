...
| 15 | Fixed / enhancement implemented | DD Overview recent transactions now switch to the DD Transactions tab and focus the matching row on double-click/activation. DD Transactions rows now open a non-modal DigiDollar details window on double-click/activation, matching the normal DGB transaction workflow while using a separate `DDTransactionDescDialog` object and green DD-specific light/dark styling. A follow-up fix applies that green style directly on the dialog widget so it cannot inherit the normal DGB blue `QDialog`/`TransactionDescDialog` styling even when the global stylesheet falls back to blue. During X11 QA this also exposed an RPC warmup `-28` table-load stall; the DD Transactions widget now falls back to direct wallet history while RPC is warming up. | `overviewRecentTransactionDoubleClickOpensTransactionsTab`, `transactionsWidgetDoubleClickShowsDetailsDialog`, `transactionsWidgetDetailsDialogOverridesDgbBlueDialogFallback`, `transactionsWidgetDetailsDialogHasDigiDollarThemeRules`, normal Qt suite, and X11 dark/light screenshots `/tmp/digibyte_dd_transaction_details_dark_qa.png` and `/tmp/digibyte_dd_transaction_details_light_qa.png`. | `d0605a19ef92410d506a7bcafeb26fd9f2ae4083`, `a28c7e2d764402f2a3d9bb614e9200ef7b924f47`, `8dcf61c097168886b0be583165f761d6b08d8537` |
...
- Qt visual QA is automated/headless only; no manual screenshot artifacts were produced.
