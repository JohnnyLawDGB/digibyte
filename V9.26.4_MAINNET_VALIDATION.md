...
Every mainnet observation matched the regtest functional suite
(`feature_digidollar_pruning.py`, 11 phases) and the code review. The pool
config in the release notes (`prune=2000`, no txindex) runs today on mainnet,
keeps the full DigiDollar-era window, and will carry a pool through
DigiDollar activation without a further upgrade.
