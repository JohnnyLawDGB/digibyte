...
MockOracleManager::MockOracleManager()
    : mockPrice(1),            // Default: $0.01 per DGB = 1 cent
      lastUpdateHeight(0),
      enabled(true)
{
}
...
void MockOracleManager::SetMockPrice(CAmount price)
{
    ...
    // Price is in DigiDollar cents (100 = $1.00)
    // Range: $0.01 per DGB (1 cent) to $1000 per DGB (100,000 cents)
    const CAmount MIN_PRICE = 1;              // 1 cent = $0.01 per DGB (minimum reasonable)
    const CAmount MAX_PRICE = 100000;         // 100,000 cents = $1000 per DGB (maximum reasonable)
    ...
    LogPrintf("MockOracleManager: Price updated to %d cents ($%.2f per DGB)\n", mockPrice, mockPrice / 100.0);
}
...
void MockOracleManager::SimulateVolatility(int percentChange)
{
    ...
    // Ensure price stays in reasonable range (DigiDollar cents: 100 = $1.00)
    ...
    LogPrintf("MockOracleManager: Simulated %d%% volatility: %d -> %d cents ($%.2f -> $%.2f per DGB)\n",
              percentChange, oldPrice, mockPrice, oldPrice / 100.0, mockPrice / 100.0);
}
...
void MockOracleManager::Reset()
{
    ...
    mockPrice = 1;  // Reset to 1 cent = $0.01 per DGB
    lastUpdateHeight = 0;
    enabled = true;

    LogPrintf("MockOracleManager: Reset to default state (price: %d cent = $0.01 per DGB)\n", mockPrice);
}