...
The public RC44 testnet roster has 21 active MuSig2 oracle slots. The local
...
class DigiDollarTestnet26OracleRosterRPCTest(DigiByteTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(deployment["oracle_pubkey_count"], 21)
        assert_equal(deployment["oracle_consensus_required"], 7)
        assert_equal(deployment["oracle_total_slots"], 35)

        self.log.info("Verify getoracles names all 21 active RC44 slots")
        active_oracles = node.getoracles(True, 20)
        assert_equal(len(active_oracles), 21)
        ...
        for oracle in active_oracles:
            ...
            assert_equal(oracle["active_oracle_count"], 21)
