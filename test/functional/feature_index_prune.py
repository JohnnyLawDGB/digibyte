    ...
    def run_test(self):
        ...
        for node in self.nodes[:2]:
            with node.assert_debug_log(['limited pruning to height 689']):
                ...
                # This boundary is sensitive to on-disk serialization details.
                assert pruneheight_new in (248, 249), pruneheight_new
        ...
        for i, msg in enumerate([filter_msg, stats_msg, filter_msg]):
            pruneheight_2 = self.nodes[i].pruneblockchain(998)
        ...
        self.log.info("reindex after pruning to the previous index tip, then rebuild indices cleanly")
        for i in range(3):
        ...
        for node in self.nodes[:2]:
            with node.assert_debug_log(['limited pruning to height 2489']):
                ...
                assert pruneheight_new in (2005, 2006), pruneheight_new
