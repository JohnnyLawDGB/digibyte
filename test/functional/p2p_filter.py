...
class FilterTest(DigiByteTestFramework):
    ...
    def test_msg_mempool(self):
        ...
        txid = self.wallet.send_to(from_node=self.nodes[0], scriptPubKey=filter_peer.watch_script_pubkey, amount=9 * COIN, fee=15000)["txid"]
    ...
    def test_frelay_false(self, filter_peer):
        ...
        self.wallet.send_to(from_node=self.nodes[0], scriptPubKey=filter_peer.watch_script_pubkey, amount=9 * COIN, fee=15000)
    ...
    def test_filter(self, filter_peer):
        ...
        filter_peer.tx_received = False
        self.wallet.send_to(from_node=self.nodes[0], scriptPubKey=getnewdestination()[1], amount=7 * COIN, fee=15000)
        filter_peer.sync_with_ping()
        ...
        filter_peer.send_and_ping(msg_filterclear())
        for _ in range(5):
            txid = self.wallet.send_to(from_node=self.nodes[0], scriptPubKey=getnewdestination()[1], amount=7 * COIN, fee=15000)["txid"]
            filter_peer.wait_for_tx(txid)
